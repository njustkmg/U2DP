...
class LanguageModelCriterion(nn.Module):
    ...
    def forward(self, input, target, mask, weights=None):
        ...
        if weights is not None:
            weights = weights.repeat_interleave(seq_len*5)  # Ensure weights match batch size
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, input, target, mask, weights=None):
        ...
        # Compute the loss
        loss = self.criterion(input, true_dist).sum(1) * mask

        # Apply sample weights
        if weights is not None:
            weights = weights.repeat_interleave(seq_len*5)
            weights = to_contiguous(weights).view(-1)  # Flatten weights
            loss = loss * weights
...
class ReduceLROnPlateau(object):
    ...
    def __init__(self, optimizer, mode='min', factor=0.1, patience=10, verbose=False, threshold=0.0001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08):
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode=mode, factor=factor, patience=patience, verbose=verbose, threshold=threshold, threshold_mode=threshold_mode, cooldown=cooldown, min_lr=min_lr, eps=eps)
        self.optimizer = optimizer
